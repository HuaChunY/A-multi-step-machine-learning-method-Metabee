...
Metabee <- function(data, 
  ...
  label<- label #facotr, "Con" is Control groups, and "tet" is experimental groups.
  ...
  oplsda = opls(input[,-1], label, predI = 1, orthoI = NA, crossvalI=5)
  
  sample.score <- oplsda@scoreMN %>%as.data.frame() %>% mutate(label = label,
  ...
  for (g in all.genes) {
    ...
    tmp <- Summarize(get(g) ~ label, data = input, digits = 3)

    
    deg.per <- try(independence_test(get(g) ~ label, 
                                     data = input,
